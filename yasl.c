...
yasljoin(char **argv, int argc, char *sep, size_t seplen) {
    ...
    for (int j = 0; j < argc; j++) {
        ...
        if (j != argc - 1) {
            join = yaslcatlen(join, sep, seplen);
        }
    }
    return join;
}
    ...
    *argc = 0;
    while (1) {
        ...
        if (*p) {
            ...
            while (!done) {
                if (inq) {
                    if (*p == '\\' && *(p + 1) == 'x' && isxdigit(*(p + 2)) &&
                            isxdigit(*(p + 3))) {
                        unsigned char byte;

                        byte = (unsigned char)((hex_digit_to_int(*(p + 2)) *
                                                       16) +
            ...
            char **tmp = realloc(
                    vector, (unsigned long)((*argc) + 1) * (sizeof(char *)));
            if (!tmp) {
                goto err;
            }
            vector = tmp;
...
yastr *
yaslsplitlen(const char *str, size_t len, const char *sep, size_t seplen,
        size_t *count) {
    if (!str || !sep || !count) {
        return NULL;
    }
...
yaslcatrepr(yastr dest, const char *src, size_t len) {
    ...
    while (len--) {
        ...
        case '\n':
            ...
            break;
        case '\r':
            dest = yaslcatlen(dest, "\\r", 2);
            break;
        case '\t':
            dest = yaslcatlen(dest, "\\t", 2);
            break;
        case '\a':
            dest = yaslcatlen(dest, "\\a", 2);
            break;
        case '\b':
            dest = yaslcatlen(dest, "\\b", 2);
            break;
        default:
            if (isprint(*src)) {
                dest = yaslcatprintf(dest, "%c", *src);
