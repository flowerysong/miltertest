...
static const luaL_Reg mt_library[] = {{"abort", mt_abort},
        {"bodyfile", mt_bodyfile}, {"bodyrandom", mt_bodyrandom},
        {"bodystring", mt_bodystring}, {"chdir", mt_chdir},
        {"connect", mt_connect}, {"conninfo", mt_conninfo}, {"data", mt_data},
        {"disconnect", mt_disconnect}, {"echo", mt_echo}, {"eoh", mt_eoh},
        {"eom", mt_eom}, {"eom_check", mt_eom_check}, {"getcwd", mt_getcwd},
        {"getheader", mt_getheader}, {"getreply", mt_getreply},
        {"header", mt_header}, {"helo", mt_helo}, {"macro", mt_macro},
        {"mailfrom", mt_mailfrom}, {"negotiate", mt_negotiate},
        {"rcptto", mt_rcptto}, {"set_timeout", mt_set_timeout},
        {"signal", mt_signal}, {"sleep", mt_sleep},
        {"startfilter", mt_startfilter}, {"test_action", mt_test_action},
...
mt_assert_state(struct mt_context *ctx, int state) {
    ...
    if (state >= STATE_NEGOTIATED && ctx->ctx_state < STATE_NEGOTIATED) {
        ...
        if (rcmd != SMFIC_OPTNEG) {
            if (verbose > 0) {
                fprintf(stdout,
                        "%s: filter returned status %d to option negotiation "
                        "on fd %d\n",
    ...
    if (state >= STATE_CONNINFO && ctx->ctx_state < STATE_CONNINFO) {
        if (!CHECK_MPOPTS(ctx, SMFIP_NOCONNECT)) {
            ...
            if (rcmd != SMFIR_CONTINUE) {
                if (verbose > 0) {
                    fprintf(stdout,
                            "%s: filter returned status %d to connection "
                            "information on fd %d\n",
                            progname, rcmd, ctx->ctx_fd);
                }
    ...
    if (state >= STATE_HEADER && ctx->ctx_state < STATE_HEADER) {
        if (!CHECK_MPOPTS(ctx, SMFIP_NOHDRS)) {
            ...
            if (rcmd != SMFIR_CONTINUE) {
                if (verbose > 0) {
                    fprintf(stdout,
                            "%s: filter returned status %d to header on fd "
                            "%d\n",
                            progname, rcmd, ctx->ctx_fd);
                }
...
mt_startfilter(lua_State *l) {
    ...
    if (argv == NULL) {
        lua_pushfstring(l, "mt.startfilter(): malloc(): %s", strerror(errno));
        lua_error(l);
    }
    ...
    if (pipe(fds) != 0) {
        lua_pushfstring(l, "mt.startfilter(): pipe(): %s", strerror(errno));
        lua_error(l);
    }

    if (fcntl(fds[ 1 ], F_SETFD, FD_CLOEXEC) != 0) {
        lua_pushfstring(l, "mt.startfilter(): fcntl(): %s", strerror(errno));
        lua_error(l);
    }
    ...
    switch (child) {
    case -1:
        lua_pushfstring(l, "mt.startfilter(): fork(): %s", strerror(errno));
        lua_error(l);
    ...
    default:
        ...
        if (c == -1) {
            lua_pushfstring(l, "mt.startfilter(): read(): %s", strerror(errno));
            lua_error(l);
        } else if (c != 0) {
            lua_pushfstring(
                    l, "mt.startfilter(): read(): got %d, expecting 0", c);
            lua_error(l);
        }
        ...
        if (child != 0) {
            lua_pushfstring(l,
                    "mt.startfilter(): wait4(): child %d exited prematurely, "
                    "status %d",
                    child, status);
            lua_error(l);
        }
...
mt_connect(lua_State *l) {
    ...
    case AF_UNIX: {
        ...
        while (count > 0) {
            ...
            if (fd < 0) {
                lua_pushfstring(
                        l, "mt.connect(): socket(): %s", strerror(errno));
                lua_error(l);
            }
            ...
            if (verbose > 1) {
                fprintf(stdout, "%s: connect(): %s; %u tr%s left\n", progname,
                        strerror(errno), count - 1, count == 2 ? "y" : "ies");
            }
        ...
        if (saverr != 0) {
            lua_pushfstring(l, "mt.connect(): %s: connect(): %s", sockinfo,
                    strerror(errno));
            lua_error(l);
        }
    ...
    if (new == NULL) {
        lua_pushfstring(l, "mt.connect(): malloc(): %s", strerror(errno));
        lua_error(l);
    }
...
mt_disconnect(lua_State *l) {
    ...
    top = lua_gettop(l);
    if ((top != 1 && top != 2) || !lua_islightuserdata(l, 1) ||
...
mt_negotiate(lua_State *l) {
    ...
    if (rcmd != SMFIC_OPTNEG) {
        if (verbose > 0) {
            fprintf(stdout,
                    "%s: filter returned status %d to option negotiation on fd "
                    "%d\n",
                    progname, rcmd, ctx->ctx_fd);
        }
    ...
    if (verbose > 0) {
        fprintf(stdout, "%s: option negotiation sent on fd %d, reply '%c'\n",
...
mt_macro(lua_State *l) {
    ...
    if (top < 4 || !lua_islightuserdata(l, 1) || !lua_isnumber(l, 2) ||
...
mt_conninfo(lua_State *l) {
    ...
    if (lua_gettop(l) != 3 || !lua_islightuserdata(l, 1) ||
    ...
    if (ipstr == NULL) {
        ...
        if (family != 'U') {
            ...
            if (inet_ntop(res->ai_family, a, tmp, sizeof tmp - 1) == NULL) {
                freeaddrinfo(res);
                lua_pushfstring(l,
                        "mt.conninfo(): can't convert address for host '%s' to "
                        "text",
                        host);
                lua_error(l);
            }
    ...
    if (verbose > 0) {
        fprintf(stdout, "%s: connection details sent on fd %d, reply '%c'\n",
                progname, ctx->ctx_fd, rcmd);
    }
...
mt_unknown(lua_State *l) {
    ...
    if (verbose > 0) {
        fprintf(stdout, "%s: UNKNOWN sent on fd %d, reply '%c'\n", progname,
                ctx->ctx_fd, rcmd);
    }
...
mt_helo(lua_State *l) {
    ...
    if (verbose > 0) {
        fprintf(stdout, "%s: HELO sent on fd %d, reply '%c'\n", progname,
                ctx->ctx_fd, rcmd);
    }
...
mt_mailfrom(lua_State *l) {
    ...
    if (verbose > 0) {
        fprintf(stdout, "%s: MAIL sent on fd %d, reply '%c'\n", progname,
                ctx->ctx_fd, rcmd);
    }
...
mt_rcptto(lua_State *l) {
    ...
    if (verbose > 0) {
        fprintf(stdout, "%s: RCPT sent on fd %d, reply '%c'\n", progname,
                ctx->ctx_fd, rcmd);
    }
...
mt_data(lua_State *l) {
    ...
    if (verbose > 0) {
        fprintf(stdout, "%s: DATA sent on fd %d, reply '%c'\n", progname,
                ctx->ctx_fd, rcmd);
    }
...
mt_header(lua_State *l) {
    ...
    if (verbose > 0) {
        fprintf(stdout, "%s: header sent on fd %d, reply '%c'\n", progname,
                ctx->ctx_fd, rcmd);
    }
...
mt_eoh(lua_State *l) {
    ...
    if (verbose > 0) {
        fprintf(stdout, "%s: EOH sent on fd %d, reply '%c'\n", progname,
                ctx->ctx_fd, rcmd);
    }
...
mt_bodystring(lua_State *l) {
    ...
    if (verbose > 0) {
        fprintf(stdout, "%s: %zu byte(s) of body sent on fd %d, reply '%c'\n",
...
mt_bodyfile(lua_State *l) {
    ...
    if (f == NULL) {
        lua_pushfstring(
                l, "mt.bodyfile(): %s: fopen(): %s", file, strerror(errno));
        lua_error(l);
    }
...
mt_eom(lua_State *l) {
    ...
    for (;;) {
        ...
        if (rcmd == SMFIR_CONTINUE || rcmd == SMFIR_ACCEPT ||
                rcmd == SMFIR_REJECT || rcmd == SMFIR_TEMPFAIL ||
                rcmd == SMFIR_DISCARD) {
            break;
        }
...
mt_eom_check(lua_State *l) {
    ...
    if (lua_gettop(l) < 2 || lua_gettop(l) > 5 || !lua_islightuserdata(l, 1) ||
            !lua_isnumber(l, 2)) {
    ...
    case MT_HDRADD: {
        ...
        for (r = ctx->ctx_eomreqs; r != NULL; r = r->eom_next) {
            if (r->eom_request == SMFIR_ADDHEADER) {
                ...
                if ((name == NULL || strcmp(name, rname) == 0) &&
                        (value == NULL || strcmp(value, rvalue) == 0)) {
                    lua_pushboolean(l, 1);
                    return 1;
                }
            }
    ...
    case MT_HDRINSERT: {
        ...
        for (r = ctx->ctx_eomreqs; r != NULL; r = r->eom_next) {
            if (r->eom_request == SMFIR_INSHEADER) {
                ...
                rvalue = r->eom_rdata + MILTER_LEN_BYTES + strlen(rname) + 1;
    ...
    case MT_QUARANTINE: {
        ...
        for (r = ctx->ctx_eomreqs; r != NULL; r = r->eom_next) {
            if (r->eom_request == SMFIR_QUARANTINE) {
                ...
                if (reason == NULL || strcmp(reason, rreason) == 0) {
                    lua_pushboolean(l, 1);
                    return 1;
                }
            }
    ...
    case MT_SMTPREPLY: {
        ...
        for (r = ctx->ctx_eomreqs; r != NULL; r = r->eom_next) {
            if (r->eom_request == SMFIR_REPLYCODE) {
                ...
                snprintf(rbuf, sizeof rbuf, "%s%s%s%s%s", smtp,
...
main(int argc, char **argv) {
    ...
    while ((c = getopt(argc, argv, CMDLINEOPTS)) != -1) {
        ...
        case 's':
            if (script != NULL) {
                fprintf(stderr, "%s: multiple use of '-%c' not permitted\n",
                        progname, c);
                lua_close(l);
                return EX_USAGE;
            }
    ...
    if (script != NULL) {
        ...
        if (rlen != s.st_size) {
            fprintf(stderr, "%s: %s: read() returned %zu (expecting %ld)\n",
...
#else  /* LUA_VERSION_NUM >= 502 */
    switch (lua_load(
            l, mt_lua_reader, (void *)&io, script == NULL ? "(stdin)" : script))
#endif /* LUA_VERSION_NUM >= 502 */
    {
    case 0:
        break;
    ...
    case LUA_ERRMEM:
        if (lua_isstring(l, 1)) {
            fprintf(stderr, "%s: %s: %s\n", progname,
                    script == NULL ? "(stdin)" : script, lua_tostring(l, 1));
        }
        lua_close(l);
        if (io.lua_io_script != NULL) {
            free((void *)io.lua_io_script);
    ...
    if (rusage) {
        ...
        if (getrusage(RUSAGE_SELF, &u) != 0) {
            fprintf(stderr, "%s: getrusage(RUSAGE_SELF): %s\n", progname,
                    strerror(errno));
    ...
    if (filterpid != 0) {
        if (kill(filterpid, SIGTERM) != 0) {
            ...
            if (verbose > 1) {
                fprintf(stdout, "%s: waiting for process %d\n", progname,
                        filterpid);
            }
            ...
            if (WIFEXITED(status) && WEXITSTATUS(status) != 0) {
                fprintf(stderr, "%s: filter process exited with status %d\n",
                        progname, WEXITSTATUS(status));

                retval = 1;
            } else if (WIFSIGNALED(status) && WTERMSIG(status) != SIGTERM) {
                fprintf(stderr, "%s: filter process died with signal %d\n",
    ...
    if (rusage && !nowait) {
        ...
        if (getrusage(RUSAGE_CHILDREN, &u) != 0) {
            fprintf(stderr, "%s: getrusage(RUSAGE_CHILDREN): %s\n", progname,
                    strerror(errno));
